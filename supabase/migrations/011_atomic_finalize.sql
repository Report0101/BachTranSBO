...
-- The Edge Function performs de-identification first, then this RPC commits the
-- completed case, its synchronized tests, finalized summary, and immutable
-- corpus revision in one PostgreSQL transaction.

create or replace function public.finalize_case_atomic(
  p_owner_id uuid,
  p_case jsonb,
  p_tests jsonb,
  p_summary jsonb,
...
begin
  ...
  loop
    ...
    v_current_test_ids := array_append(
      v_current_test_ids,
      (v_test->>'id')::uuid
    );
  ...
  select s.model, s.skill_version
    into v_model, v_skill_version
...
revoke all on function public.finalize_case_atomic(
  uuid,
  jsonb,
  jsonb,
  jsonb,
...
grant execute on function public.finalize_case_atomic(
  uuid,
  jsonb,
  jsonb,
  jsonb,
